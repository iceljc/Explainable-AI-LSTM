...
def deletion_iter(num_del, method, data_iter, net):
	...
	for i, batch in enumerate(data_iter):
		...
		if len(src) >= 10:
			...
			if pred_label == true_label:
				...
				# random deletion
				del_ids = random.sample(range(0, len(src)), num_del)
				del_ids = np.array(del_ids)

				net.set_input(src, delete_ids=del_ids)
...
def run_lrp(data_iter, vocab, model_file):
	...
	while num_del <= 5:
		...
		for it in range(iters):
			...
			count1, count2, correct_total, incorrect_total = deletion_iter(num_del, method, iterator, net)
